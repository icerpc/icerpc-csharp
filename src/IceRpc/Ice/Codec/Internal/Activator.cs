...
internal class ActivatorFactory
{
    ...
    internal Activator Get(Assembly assembly)
    {
        ...
        else if (assembly.GetCustomAttributes<IceAttribute>().Any())
        {
            return _cache.GetOrAdd(
                ...
                assembly =>
                {
                    ...
                    {
                        // We're only interested in generated Ice classes and exceptions.
                        if (type.IsClass && type.GetIceTypeId() is string typeId)
                        {
                            dict.Add(typeId, type);

                            if (type.GetCompactIceTypeId() is int compactTypeId)
                            {
        ...
        {
            // We don't cache an assembly with no Ice attribute, and don't load/process its referenced assemblies.
