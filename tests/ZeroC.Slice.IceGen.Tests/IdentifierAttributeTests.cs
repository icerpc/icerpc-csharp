// Copyright (c) ZeroC, Inc.

using NUnit.Framework;
using ZeroC.Slice.Codec;

namespace ZeroC.Slice.IceGen.Tests.Identifiers;

/// <summary>These tests verify that the cs::identifier attribute will cause the Ice Slice compiler to generate C#
/// with the specified identifiers.</summary>
[Parallelizable(scope: ParallelScope.All)]
public class IdentifierAttributeTests
{
    [Test]
    public void Renamed_exception()
    {
        // Act
        _ = new REnamedException();

        // Assert
        Assert.That(typeof(REnamedException).GetSliceTypeId(), Is.EqualTo("::ZeroC::Slice::IceGen::Tests::OriginalException"));
    }

    [Test]
    public void Renamed_class_with_renamed_field()
    {
        // Act / Assert
        var myClass = new REnamedClass(1);

        // Assert
        Assert.That(myClass.renamedX, Is.EqualTo(1));
    }
}
