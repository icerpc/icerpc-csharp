    ...
    extension(ref SliceDecoder decoder)
    {
        ...
        private ServerAddress DecodeServerAddress(Protocol protocol)
        {
            ...
            if (encodingMajor == 1 && encodingMinor <= 1)
            {
                ...
                if (serverAddress is not null)
                {
                    ...
                    {
                        throw new InvalidDataException(
                            $"There are {oldPos + size - decoder.Consumed} bytes left in server address " +
                            "encapsulation.");
                    }
                }
            }
            ...
            {
                throw new InvalidDataException(
                    $"Cannot decode server address for protocol '{protocol}' and transport " +
                    $"'{transportCode.ToString().ToLowerInvariant()}' with server address encapsulation encoded " +
                    $"with encoding '{encodingMajor}.{encodingMinor}'.");
            }
        ...
        private ServiceAddress DecodeServiceAddressCore(string path)
        {
            ...
            else
            {
                ...
                if (count >= 2)
                {
                    // A Slice1 encoded server address consumes at least 8 bytes (2 bytes for the server address
                    // type and 6 bytes for the encapsulation header). SizeOf ServerAddress is large but less
                    // than 8 * 8.
        ...
        private ServerAddress DecodeTcpServerAddressBody(string transport)
        {
            Debug.Assert(decoder.Encoding == SliceEncoding.Slice1);
