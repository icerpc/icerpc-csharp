...
namespace IceRpc.Tests.ClientServer
{
    ...
    public class TlsConfigurationTests : ClientServerBaseTest
    {
            ...
            string caFile)
        {
            ...
            {
                ClientTransport = new TcpClientTransport(authenticationOptions:
                    new()
                    {
        ...
        public async Task TlsConfiguration_Fail_WithUntrustedClient(string clientCertFile, string caFile)
        {
            ...
            await using var connection = new Connection
            {
                ClientTransport = new TcpClientTransport(authenticationOptions: tlsClientOptions),
                RemoteEndpoint = server.Endpoint
            };
            var prx = ServicePrx.FromConnection(connection);
        ...
        public async Task TlsConfiguration_With_CommonProtocol()
        {
            ...
            SslStream? sslStream =
                (prx.Proxy.Connection!.TransportConnection as NetworkSocketConnection)?.NetworkSocket.SslStream;
        ...
        private Server CreateServer(string? hostname, SslServerAuthenticationOptions tlsServerOptions)
        {
            ...
            var server = new Server
            {
                ...
                ServerTransport = new TcpServerTransport(authenticationOptions: tlsServerOptions)
            };
