...
public static class QuicTransportServiceCollectionExtensions
{
    ...
    /// <remarks>On macOS, the kernel can give msquic's dual-mode listener socket an ephemeral port that an IPv4-only
    /// socket of another process already holds. IPv4 datagrams to that port are then delivered to the other process
    /// and the handshake times out. Connecting over IPv6 avoids this misdelivery. See
