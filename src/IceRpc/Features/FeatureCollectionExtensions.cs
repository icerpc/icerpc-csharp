...
{
    /// <summary>Extension methods for <see cref="IFeatureCollection" />.</summary>
    /// <param name="features">This feature collection.</param>
    extension(IFeatureCollection features)
    {
        ...
        public IFeatureCollection AsReadOnly() =>
        ...
        public IFeatureCollection With<T>(T value)
        {
            var result = features;
            if (result.IsReadOnly)
            {
                result = new FeatureCollection(result);
            }
            result.Set(value);
            return result;
        }
