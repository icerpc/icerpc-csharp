...
public class ServerAddressFeatureTests
{
    ...
    {
        var removedServerAddresses = new ServerAddress[] { ServerAddress.FromUri(new Uri("icerpc://127.0.0.1:10001")) };
        var serviceAddress = ServiceAddress.FromUri(new Uri("icerpc://127.0.0.1:10001/hello"));
    ...
    {
        var removedServerAddresses = new ServerAddress[] { ServerAddress.FromUri(new Uri("icerpc://127.0.0.1:10001")) };
        var serviceAddress = ServiceAddress.FromUri(new Uri("icerpc://127.0.0.1:10001/hello?alt-server=127.0.0.1:10002"));
        var serverAddressFeature = new ServerAddressFeature(serviceAddress);

        serverAddressFeature.RemoveServerAddress(removedServerAddresses.First());

        Assert.That(serverAddressFeature.ServerAddress, Is.EqualTo(ServerAddress.FromUri(new Uri("icerpc://127.0.0.1:10002"))));
        Assert.That(serverAddressFeature.AltServerAddresses, Is.Empty);
        Assert.That(serverAddressFeature.RemovedServerAddresses, Is.EqualTo(removedServerAddresses));
    }
