...
public partial class ServiceTests
{
    ...
    [TestCase(typeof(MyWidgetService), "/icerpc.protobuf.tests.MyWidget")]
    ...
    public void Router_map_with_an_interface_succeeds() =>
        Assert.That(
            () => new Router().Map<IMyFirstWidgetService>(new MultipleServices()),
            Throws.Nothing);
    ...
    [SuppressMessage(
        ...
        Justification = "This class is used to verify code generation for a service with no RPC methods.")]
    internal partial class MyWidgetService : IMyWidgetService
    {
        // MyWidget does not define any RPC methods.
    }
