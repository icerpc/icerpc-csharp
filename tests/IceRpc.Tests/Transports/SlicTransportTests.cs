...
public class SlicTransportTests
{
    ...
    public async Task Stream_peer_options_are_set_after_connect()
    {
        ...
        await using ServiceProvider provider = new ServiceCollection()
            .AddSingleton<IServerTransport<IMultiplexedTransportConnection>>(
            ...
            .AddSingleton<IClientTransport<IMultiplexedTransportConnection>>(
        ...
        var clientConnection = provider.GetRequiredService<SlicTransportConnection>();
        var listener = provider.GetRequiredService<IListener<IMultiplexedTransportConnection>>();
        Task<IMultiplexedTransportConnection> acceptTask = ConnectAndAcceptConnectionAsync(listener, clientConnection);

        // Act
        var serverConnection = (SlicTransportConnection)await acceptTask;
        ...
        int pauseThreshold)
    {
        ...
        var listener = provider.GetRequiredService<IListener<IMultiplexedTransportConnection>>();
        Task<IMultiplexedTransportConnection> acceptTask = ConnectAndAcceptConnectionAsync(listener, clientConnection);
        await using IMultiplexedTransportConnection serverConnection = await acceptTask;
