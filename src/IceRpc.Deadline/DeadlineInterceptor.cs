...
/// <summary>The deadline interceptor provides a mechanism to set a request deadline also known as invocation timeout.
/// If the request doesn't carry a deadline feature a deadline will be computed based on the configured default
/// timeout otherwise the deadline from the <see cref="IDeadlineFeature"/> feature is used, the deadline is then
/// encoded as a request field using the <see cref="RequestFieldKey.Deadline"/> key. When the deadline expires, the
/// invocation is canceled and the interceptor throws <see cref="TimeoutException" />.
/// When used in conjunction with the retry interceptor it is important to install this interceptor before the retry
/// interceptor to ensure that the deadline is computed once per invocation and not once for each retry.</summary>
