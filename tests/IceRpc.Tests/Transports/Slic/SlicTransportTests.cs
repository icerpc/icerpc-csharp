...
public class SlicTransportTests
{
    ...
    /// <summary>Verifies that the connection-level outbound pipe is bounded by PauseWriterThreshold. A write
    /// decorator on the underlying duplex connection tracks the maximum payload bytes observed in a single write,
    /// which must not exceed PauseWriterThreshold plus frame header overhead.</summary>
    [Test]
    public async Task Connection_outbound_buffer_does_not_exceed_pause_writer_threshold()
    {
        ...
        // Act: write a payload much larger than PauseWriterThreshold. Each duplex write reads whatever is
        // currently buffered in the connection-level pipe, which is bounded by PauseWriterThreshold.
        ...
        // Assert: the maximum bytes written to the duplex connection in a single call should not significantly
        // exceed PauseWriterThreshold. We allow a margin for Slic frame headers (frame type + size + stream ID
        // per frame, typically ~10 bytes each).
        Assert.That(
