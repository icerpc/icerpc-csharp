...
- The Slice compiler, `slicec`, which parses and validates Slice files.
- The `ZeroC.Slice.Generator`, which generates C# code for Slice types (encoding, decoding, etc.).
- The `IceRpc.Slice.Generator`, which generates C# code for IceRPC-specific constructs (proxies, dispatchers, etc.).
...
> When the Slice compiler compiles a Slice file and the resulting .cs file is identical to the existing output .cs file,
...
Slice compiler, you should include the corresponding Slice files in your NuGet package. This allows consumers of your
NuGet package to easily retrieve these Slice files.
...
| IceRpc            | `true`    | When `true`, the IceRpc.Slice.Generator runs in addition to the ZeroC.Slice.Generator, generating IceRPC-specific code (proxies, dispatchers). When `false`, only the ZeroC.Slice.Generator runs. |
...
|--------------------------------|----------------------------------------|
| `true` or no `IceRpc` metadata | [IceRpc.Slice][icerpc-slice]           |
| `false`                        | [ZeroC.Slice.Codec][zeroc-slice-codec] |
