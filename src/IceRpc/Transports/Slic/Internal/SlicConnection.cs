...
internal class SlicConnection : IMultiplexedConnection
{
    ...
    /// <summary>Gets the pause writer threshold for the connection's outbound pipe.</summary>
...
#pragma warning restore CA2213
                ...
                if (IsServer)
                {
                    ...
                    if (initializeBody is null)
                    {
                        ...
                        await WriteConnectionFrameAsync(
                            FrameType.Version,
                            new VersionBody(supportedVersions).Encode,
                            cancellationToken).ConfigureAwait(false);
        ...
        if (TryClose(new IceRpcException(IceRpcError.OperationAborted), "The connection was closed."))
        {
            ...
            try
            {
                ...
                lock (_mutex)
                {
                    ...
                    // (_writerIsShutdown=true) if the client-side sent the Close frame and shut down the duplex
                    // connection. This doesn't apply to the client-side since the server-side doesn't shutdown the
                    // duplex connection writer after sending the Close frame.
                    if (!IsServer || !_writerIsShutdown)
                    {
                        ...
                        else
                        {
                            ...
                            // connection. For TCP, it's important to always shutdown the connection on the
                            // client-side first to avoid TIME_WAIT states on the server-side.
        ...
        bool isServer)
    {
        ...
        {
            // Fire-and-forget: timer callbacks cannot await. The synchronous prefix runs up to the WaitAsync inside
            // WriteConnectionFrameAsync, which atomically queues this writer.
            _ = SendPingAsync();
    ...
    /// <param name="cancellationToken">A cancellation token that receives the cancellation requests.</param>
    internal async ValueTask WriteConnectionFrameAsync(
        FrameType frameType,
        EncodeAction? encode,
        CancellationToken cancellationToken)
    {
    ...
    /// <see cref="SlicPipeReader.Complete"/>, <see cref="SlicPipeWriter.Complete"/>, window updates) that cannot
    /// await.</summary>
        ...
        bool writeReadsClosedFrame)
    {
        ...
        // SemaphoreSlim.WaitAsync atomically updates the semaphore state (acquires it or enqueues the waiter)
        // synchronously, before the await can yield. Two sequential calls from the same thread therefore enqueue
        // in call order, preserving wire ordering of the resulting frames.
        _ = WriteStreamFrameAsync();

        async Task WriteStreamFrameAsync()
        {
            await _writeSemaphore.WaitAsync(_closedCancellationToken).ConfigureAwait(false);
            try
            {
                lock (_mutex)
                {
                ...
                {
                    // The connection was closed while flushing.
                }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            do
            {
                ...
                // or PauseWriterThreshold. Bounding by PauseWriterThreshold ensures the connection-level outbound
                // pipe never holds more than one threshold-sized frame at a time, so a single duplex write carries
                // at most ~PauseWriterThreshold bytes.
                int sendMaxSize = Math.Min(Math.Min(sendCredit, PeerMaxStreamFrameSize), PauseWriterThreshold);
                ...
                try
                {
                    ...
                    // reached — the connection's write semaphore is held during the await, so all other connection
                    // writers wait until the background writer task drains enough data.
            ...
            if (IsServer)
            {
                ...
                try
                {
                    lock (_mutex)
                    {
                        ...
                        // Make sure that CloseAsync doesn't call Write on the writer if it's called shortly after the
                        // peer shutdown its side of the connection (which triggers ReadFrameHeaderAsync to return
                        // null).
