...
mod tests {
    ...
    {
        op(a: AStruct) -> AStruct;
    }"#;

    const SLICE_WITH_CLASS: &str = r#"
    module Test;
    ...
    const INTERFACE_IDENTIFIER: &str = "::Test::TestInterface";

    #[test]
    fn expected_request_class() {
        let ast = parse_string(SLICE_WITH_STRUCT).unwrap();
        let interface_ptr = ast.find_typed_entity::<Interface>(INTERFACE_IDENTIFIER).unwrap();
        let interface = interface_ptr.borrow();

        let request_class = request_class(interface).to_string();

        let expected = r#"
/// <summary>Converts the arguments of each operation that takes arguments into a request payload.</summary>
public static class Request
{
    /// <summary>Creates the request payload for operation op.</summary>
    /// <param name="encoding">The encoding of the payload.</param>
    /// <returns>The payload encoded with <paramref name="encoding"/>.</returns>
    public static global::System.IO.Pipelines.PipeReader Op(
        ...
        AStruct a)
    {
        ...
        assert_eq!(expected.trim(), request_class.trim());
    }

    #[test]
    fn expected_request_class_when_sending_classes() {
        let ast = parse_string(SLICE_WITH_CLASS).unwrap();
        let interface_ptr = ast.find_typed_entity::<Interface>(INTERFACE_IDENTIFIER).unwrap();
        let interface = interface_ptr.borrow();
...
{
    /// <summary>Creates the request payload for operation op.</summary>
    /// <returns>The payload encoded with encoding 1.1.</returns>
    public static global::System.IO.Pipelines.PipeReader Op(AClass a)
    {
        var pipe_ = new global::System.IO.Pipelines.Pipe();
        var encoder_ = new SliceEncoder(pipe_.Writer, IceRpc.Encoding.Slice11, default);
        Span<byte> sizePlaceholder_ = encoder_.GetPlaceholderSpan(4);
        int startPos_ = encoder_.EncodedByteCount;
        encoder_.EncodeClass(a);
        SliceEncoder.EncodeVarULong((ulong)(encoder_.EncodedByteCount - startPos_), sizePlaceholder_);
    ...
    #[test]
    fn expected_response_class() {
        let ast = parse_string(SLICE_WITH_STRUCT).unwrap();
        let interface_ptr = ast.find_typed_entity::<Interface>(INTERFACE_IDENTIFIER).unwrap();
        let interface = interface_ptr.borrow();

        let response_class = response_class(interface).to_string();

        let expected = r#"
/// <summary>Holds a <see cref="IceRpc.Slice.ResponseDecodeFunc{T}"/> for each non-void remote operation defined in <see cref="ITestInterfacePrx"/>.</summary>
public static class Response
{
    /// <summary>The <see cref="ResponseDecodeFunc{T}"/> for the return value type of operation op.</summary>
    public static async global::System.Threading.Tasks.ValueTask<AStruct> OpAsync(
        IceRpc.IncomingResponse response,
        global::System.Threading.CancellationToken cancel) =>
        await response.ToReturnValueAsync(
            _defaultActivator,
            (ref SliceDecoder decoder) => new AStruct(ref decoder),
            hasStream: false,
            cancel).ConfigureAwait(false);
}"#;

        assert_eq!(expected.trim(), response_class.trim());
    }

    #[test]
    fn expected_response_class_when_sending_classes() {
        let ast = parse_string(SLICE_WITH_CLASS).unwrap();
        let interface_ptr = ast.find_typed_entity::<Interface>(INTERFACE_IDENTIFIER).unwrap();
        let interface = interface_ptr.borrow();
...
{
    /// <summary>The <see cref="ResponseDecodeFunc{T}"/> for the return value type of operation op.</summary>
    public static async global::System.Threading.Tasks.ValueTask<AClass> OpAsync(
        IceRpc.IncomingResponse response,
        global::System.Threading.CancellationToken cancel) =>
        await response.ToReturnValueAsync(
            _defaultActivator,
            (ref SliceDecoder decoder) => decoder.DecodeClass<AClass>(),
            hasStream: false,
            cancel).ConfigureAwait(false);
}"#;
