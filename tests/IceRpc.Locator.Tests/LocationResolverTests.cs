...
public class LocationResolverTests
{
    ...
    public async Task Endpoint_finder_not_called_when_cache_entry_age_is_less_or_equal_than_refresh_threshold(
        ...
        int cacheEntryAge)
    {
        var cachedProxy = ServiceAddress.Parse("ice://localhost/cached");
    ...
    {
        var cachedProxy = ServiceAddress.Parse("ice://localhost/cached");
        var resolvedProxy = ServiceAddress.Parse("ice://localhost/resolved");
    ...
    {
        var cachedProxy = ServiceAddress.Parse("ice://localhost/stale");
        var resolvedProxy = ServiceAddress.Parse("ice://localhost/resolved");
        var endpointFinder = new MockEndpointFinder(resolvedProxy);
        var resolver = new LocationResolver(
                endpointFinder,
    ...
    {
        var wellKnownProxy = ServiceAddress.Parse("ice:/foo?adapter-id=bar");
        var adapterIdProxy = ServiceAddress.Parse("ice://localhost/resolved");
    ...
    private class MockEndpointFinder : IEndpointFinder
    {
        ...
        private readonly ServiceAddress? _adapterIdProxy;
        private readonly ServiceAddress? _wellKnownProxy;

        internal MockEndpointFinder(
            ServiceAddress? wellKnownProxy = null,
            ServiceAddress? adapterIdProxy = null)
        {
    ...
    private class MockEndpointCache : IEndpointCache
    {
        ...
        public void Set(Location location, ServiceAddress proxy) => throw new NotImplementedException();
