...
fn encode_type(
    ...
    match &type_ref.concrete_typeref() {
        TypeRefs::Interface(_) if type_ref.is_optional && encoding == Encoding::Slice1 => {
            format!(
                "{encoder_param}.EncodeNullableProxy({param}?.ServiceAddress);",
        ...
        concrete_typeref => {
            ...
            let encode_type = match concrete_typeref {
                ...
                TypeRefs::Interface(_) => {
                    format!("{encoder_param}.EncodeProxy({value}.ServiceAddress);", encoder_param = encoder_param, value = value)
                }
    ...
    match &type_ref.concrete_typeref() {
        TypeRefs::Interface(_) => {
            if is_optional && encoding == Encoding::Slice1 {
                write!(
                    code,
                    "(ref SliceEncoder encoder, {value_type} value) => encoder.EncodeNullableProxy(value?.ServiceAddress)",
                    value_type = value_type,
                );
            } else {
                write!(
                    code,
                    "(ref SliceEncoder encoder, {value_type} value) => encoder.EncodeProxy({value}.ServiceAddress)",
                    value_type = value_type,
                    value = value
                );
            }
