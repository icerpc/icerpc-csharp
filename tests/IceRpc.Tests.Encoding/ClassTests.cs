    ...
    public class ClassTests
    {
        ...
        public async Task Class_FormatMetadata()
        {
            ...
            pipeline1.Use(next => new InlineInvoker(async (request, cancel) =>
            {
                ...
                var istr = new BufferReader(data, prx1.Encoding);
                ...
                istr = new BufferReader(await response.GetPayloadAsync(cancel), prx1.Encoding);
                if (prx1.Encoding == IceRpc.Encoding.V20)
                {
                    // Read the compression status '0' not compressed
                    Assert.AreEqual(0, istr.ReadByte());
            ...
            {
                ReadOnlyMemory<byte> data = request.Payload.ToSingleBuffer();
                var istr = new BufferReader(data, prx2.Encoding);
                if (prx1.Encoding == IceRpc.Encoding.V20)
                {
                    // Read the compression status '0' not compressed
                    Assert.AreEqual(0, istr.ReadByte());
            ...
            {
                ReadOnlyMemory<byte> data = request.Payload.ToSingleBuffer();
                var istr = new BufferReader(data, prx3.Encoding);
                if (prx1.Encoding == IceRpc.Encoding.V20)
                {
                    // Read the compression status '0' not compressed
                    Assert.AreEqual(0, istr.ReadByte());
