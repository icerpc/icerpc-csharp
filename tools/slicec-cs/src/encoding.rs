...
fn encode_type(
    ...
    match &type_ref.concrete_typeref() {
        TypeRefs::Interface(_) if type_ref.is_optional && encoding == Encoding::Slice1 => {
            format!("IceRpc.ServiceAddressSliceEncoderExtensions.EncodeNullableServiceAddress({encoder_param}, {param}?.ServiceAddress);")
        }
        ...
        concrete_typeref => {
            ...
            let encode_type = match concrete_typeref {
                ...
                TypeRefs::Interface(_) => format!("IceRpc.ServiceAddressSliceEncoderExtensions.EncodeServiceAddress({encoder_param}, {value}.ServiceAddress);"),
    ...
    match &type_ref.concrete_typeref() {
        TypeRefs::Interface(_) => {
            if is_optional && encoding == Encoding::Slice1 {
                write!(code, "IceRpc.ServiceAddressSliceEncoderExtensions.EncodeNullableServiceAddress(encoder, value?.ServiceAddress)");
            } else {
                write!(
                    code,
                    "IceRpc.ServiceAddressSliceEncoderExtensions.EncodeServiceAddress(encoder, {value}.ServiceAddress)"
                );
            }
        }
        TypeRefs::Class(_) => {
