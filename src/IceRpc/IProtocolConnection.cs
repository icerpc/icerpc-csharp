...
public interface IProtocolConnection : IInvoker, IAsyncDisposable
{
    ...
    /// and a task that completes when the connection itself wants to be shut down then disposed by the caller.
    /// </returns>
    /// <remarks><para>The shutdown-requested task can complete when the peer initiates a shutdown, when the connection
    /// is inactive for too long (see <see cref="ConnectionOptions.InactivityTimeout" />), when the connection detects a
    /// protocol violation, or when the connection gets an error from its transport connection.</para>
