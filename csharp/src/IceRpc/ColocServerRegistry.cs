...
namespace IceRpc
{
    ...
    internal static class ColocServerRegistry
    {
        private static readonly object _mutex = new();
        private static volatile ImmutableList<Server> _serverList = ImmutableList<Server>.Empty;

        internal static Endpoint? GetColocEndpoint(ServicePrx proxy) =>
            _serverList.Select(server => server.GetColocEndpoint(proxy)).
